...
# Heavy tool installs (uv and pi-coding-agent) are baked into image. Pi packages
# are installed here so they land in persisted PI_CODING_AGENT_DIR volume.
...
if [ -e .venv ]; then
  echo ".venv already exists; leaving it unchanged"
else
  uv venv
fi
